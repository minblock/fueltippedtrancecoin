...
const char * const BITCOIN_CONF_FILENAME = "fueltippedtrancecoin.conf";
...
#else
    const char* pszModule = "fueltippedtrancecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Fueltippedtrancecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Fueltippedtrancecoin";
#else
    // Unix
    return pathRet / ".fueltippedtrancecoin";
#endif
#endif
}
